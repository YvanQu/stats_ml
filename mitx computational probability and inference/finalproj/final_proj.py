...
def compute_empirical_distribution(values):
    ...
    """
    distribution = {value: 0 for value in set(values)}#collections.defaultdict(lambda: 0)

    # -------------------------------------------------------------------------
    # YOUR CODE HERE
    #
    for value in values:
        distribution[value] += 1/len(values)
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def compute_empirical_mutual_info_nats(var1_values, var2_values):
    ...
    for (var1, var2), prob in joint_distribution.items():
        empirical_mutual_info_nats += prob * (np.log(prob) - np.log(var1_distribution[var1]) - np.log(var2_distribution[var2]))
...
def chow_liu(observations):
    ...
    # YOUR CODE HERE
    #
    nodes = range(num_vars)
    possible_edges = []
    for node in nodes:
        possible_edges.extend(zip(range(node), [node] * node))
    possible_edges.sort(key= lambda edge: compute_empirical_mutual_info_nats(observations[:, edge[0]], observations[:, edge[1]]))
    for n in range(num_vars - 1):
        edge = possible_edges.pop()
        while union_find.find(edge[0]) == union_find.find(edge[1]):
              edge = possible_edges.pop()
        best_tree.add(edge)
...
def compute_empirical_conditional_distribution(var1_values, var2_values):
    ...
    for x2 in conditional_distributions:
        conditioned_var1_values = []
        for n in range(len(var1_values)):
            if var2_values[n] == x2:
                conditioned_var1_values.append(var1_values[n])
        conditional_distributions[x2] = compute_empirical_distribution(conditioned_var1_values)
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def learn_tree_parameters(observations, tree, root_node=0):
    ...
    # YOUR CODE HERE
    #
    node_potentials = {node: {value: 1 for value in set(observations[:, node])} for node in nodes if node != root_node}
    node_potentials[root_node] = compute_empirical_distribution(observations[:, root_node])
    # edge_potentials = collections.defaultdict(lambda edge: transpose_2d_table(edge_potentials[(edge[1], edge[0])]))
    ...
    edge_potentials.update({(edge[1], edge[0]): transpose_2d_table(potential) for edge, potential in edge_potentials.items()})
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def sum_product(nodes, edges, node_potentials, edge_potentials):
    ...
    nodes.add(root_node)
    for node in nodes:
        if len(edges[node]) > len(edges[root_node]):
            root_node = node
    
    xis = node_potentials
    leaves = [node for node in nodes if len(edges[node]) == 1 and node != root_node]

    def calculate_xi(node):
        for value in node_potentials[node]:
            for message in messages[node].values():
                xis[node][value] *= message[value]
    while leaves:
        node = leaves.pop(0)
        if len(messages[node]) < len(edges[node]) - 1:
            leaves.append(node)
        elif node == root_node:
            if len(messages[node]) == len(edges[node]):
                calculate_xi(node)
                leaves = []
            else:
                leaves.append(node)
            ...
            calculate_xi(node)
            for neighbor in edges[node]:
                if neighbor not in messages[node]:
                    parent = neighbor
            message = {x_i: 0 for x_i in node_potentials[parent]}
            for x_j, xi in xis[node].items():
                for x_i in message:
                    message[x_i] += xi * edge_potentials[(parent, node)][x_i][x_j]
            messages[parent][node] = message
            leaves.append(parent)

    marginals[root_node] = xis[root_node]
    
    fringe = [root_node]  # this is a list of nodes queued up to be visited next
    while len(fringe) > 0:
        node = fringe.pop(0)  # removes the 0th element of `fringe` and returns it
        for child, up_message in messages[node].items():
                # do some processing that involves the edge `(node, neighbor)` here
            down_message = {x_j: 0 for x_j in node_potentials[child]}
            for x_i, p in marginals[node].items():
                for x_j in down_message:
                    down_message[x_j] += p * edge_potentials[(node, child)][x_i][x_j] / up_message[x_i]
            marginals[child] = {x_j: m * xis[child][x_j] for x_j, m in down_message.items()}
                # finally after you do your processing, add `neighbor` to `fringe`
            fringe.append(child)

    for node, marginal in marginals.items():
        z = np.sum(list(marginal.values()))
        marginals[node] = {x_j: p / z for x_j, p in marginal.items() if p > 1e-8}
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def compute_marginals_given_observations(nodes, edges, node_potentials,
    ...
    # YOUR CODE HERE
    #
    new_node_potentials = node_potentials#= {node: compute_empirical_distribution(observations[:, node]) for node in nodes}
    for node, observed_value in observations.items():
        new_node_potentials[node] = {value: 0 for value in node_potentials[node]}
        new_node_potentials[node] = {observed_value: 1}
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
