...
def get_log_probabilities(file_list):
    ...
    # raise NotImplementedError
    log_n_smoothed = np.log(len(file_list) + 2)
    log_probabilities = {word: np.log(count + 1) - log_n_smoothed for word, count in get_counts(file_list).items()}
    return collections.defaultdict(lambda: -log_n_smoothed, log_probabilities)
...
def learn_distributions(file_lists_by_category):
    ...
    # raise NotImplementedError
    # log_probabilities_by_category = [get_log_probabilities(list) for list in file_lists_by_category]
    # log_n = np.log(len(file_lists_by_category[0]) + len(file_lists_by_category[1]))
    # log_prior_by_category = [np.log(len(list)) - log_n for list in file_lists_by_category]
    # return log_probabilities_by_category, log_prior_by_category
    log_probabilities_by_category = []
    log_prior_by_category = []    
    log_n = np.log(len(file_lists_by_category[0]) + len(file_lists_by_category[1]))
    for list in file_lists_by_category:
        log_probabilities_by_category.append(get_log_probabilities(list))
        log_prior_by_category.append(np.log(len(list)) - log_n)
    return log_probabilities_by_category, log_prior_by_category
...
def classify_email(email_filename,
    ...
    #     return "ham"
    # return 'spam'
    decider = log_prior_by_category[0] - log_prior_by_category[1]
    words_in_email = set(util.get_words_in_file(email_filename))
    for word in words_in_email:
        decider += log_probabilities_by_category[0][word] - log_probabilities_by_category[1][word]
    for word in log_probabilities_by_category[0]:
        if word not in words_in_email:
            decider += np.log(1 - np.exp(log_probabilities_by_category[0][word]))
    for word in log_probabilities_by_category[1]:
        if word not in words_in_email:
            decider -= np.log(1 - np.exp(log_probabilities_by_category[1][word]))
    if decider < 0: return 'ham'
    return 'spam'
